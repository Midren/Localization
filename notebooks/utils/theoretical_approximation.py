...
def get_intervals_with_same_rssi(rssi_bins, dist_bins):
    ...
    return np.array(
        [(min(distances), max(distances)) for distances in rssi_to_dist_map.values()]
    )
...
def get_radiuses_for_distance_contour_lines(rssi_accuracy, max_distance=10):
    ...
    return get_radiuses_for_distance_contour_lines_from_intevals(
        get_intervals_with_same_rssi(rssi_bins, distances)
    )
...
def drop_small_radiuses_groups(points_grouped, radiuses, minimum_radius=0.1):
    ...
    return dict(
        filter(
            lambda x: all(map(lambda x: x >= min_group, x[0])), points_grouped.items()
        )
    )
...
def accuracy_by_box(points):
    x_boundary = np.array(
        [min(points, key=lambda x: x[0])[0], max(points, key=lambda x: x[0])[0]]
    )
    y_boundary = np.array(
        [min(points, key=lambda x: x[1])[1], max(points, key=lambda x: x[1])[1]]
    )

    minima, maxima = np.concatenate(
        [x_boundary.reshape(1, -1), y_boundary.reshape(1, -1)]
    ).T
...
def calculate_per_region_accuracy(points_grouped, calculate_accuracy):
    return {
        region: calculate_accuracy(points) for region, points in points_grouped.items()
    }
...
def group_points_by_region(points, point_flags):
    ...
    return {
        group: np.array(points_grouped_list[group])
        for group in points_grouped_list.keys()
    }


def calculate_mean_accuracy(x_len, y_len, beacons_location, radiuses, delta=0.01):
    points = cartesian_cross_product(
        np.arange(0, x_len, delta), np.arange(0, y_len, delta)
    )
    point_flags = find_point_flag(points, beacons_location, radiuses)

    points_grouped = drop_small_radiuses_groups(
        group_points_by_region(points, point_flags), radiuses
    )

    return mean_accuracy(calculate_per_region_accuracy(points_grouped, accuracy_by_box))


def draw_theoretical_accuracies(x_len, y_len, beacons_location, radiuses, delta=0.01):
    points = cartesian_cross_product(
        np.arange(0, x_len, delta), np.arange(0, y_len, delta)
    )
    point_flags = find_point_flag(points, beacons_location, radiuses)
    ...
    per_region_accuracy = calculate_per_region_accuracy(points_grouped, accuracy_by_box)

    field = np.zeros((y_len * 100, x_len * 100))
    for reg, points in points_grouped.items():
        for point in points:
            field[int(round(point[1] * 100))][
                int(round(point[0] * 100))
            ] = per_region_accuracy[reg]
